    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        commonMap.forEach { map ->
            ...
            when {
                includeUrlsSize > 0 -> {
                    val isIncludeUrl =
                        !includeUrls.contains(request.url.toUrl().path) && !includeUrls.contains(
                            url.toString()
                        ) && includeUrls.none {
                            url.toString().endsWith(it)
                        } && includeUrls.none {
                ...
                excludeUrlsSize > 0 -> {
                    val isExcludeUrl =
                        !excludeUrls.contains(request.url.toUrl().path) && !excludeUrls.contains(
                            url.toString()
                        ) && excludeUrls.none {
                            url.toString().endsWith(it)
                        } && excludeUrls.none {
